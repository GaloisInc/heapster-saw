...
instance PermCheckExtC ext =>
  ...
  translate (mbMatch -> [nuMP| TypedJumpTarget entryID _ _ ghosts perms |]) =
...
tcTranslateCFGTupleFun w env endianness cfgs_and_perms =
  let lrts = map (someCFGAndPermLRT env) cfgs_and_perms
      ...
      fakeEnv = permEnvAddGlobalSyms env $
                  map (\cfg_and_perm ->
                        PermEnvGlobalEntry
      ...
      tcfgs = map (\cfg_and_perms ->
                      case cfg_and_perms of
                        SomeCFGAndPerm _ _ cfg fun_perm ->
                          SomeTypedCFG $ tcCFG fakeEnv endianness fun_perm cfg) cfgs_and_perms
